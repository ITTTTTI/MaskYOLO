...
def run(
    ...
    for batch_i, (im,im_mask, targets, paths, shapes) in enumerate(pbar):
        callbacks.run('on_val_batch_start')
        t1 = time_sync()
        if cuda:
            im = im.to(device, non_blocking=True)
            targets = targets.to(device)
            im_mask=im_mask.to(device, non_blocking=True)
        ...
        dt[0] += t2 - t1
        # Inference
        out,img_mask = model(im) if training else model(im, augment=augment, val=True)  # inference, loss outputs
        out,train_out=out
        dt[1] += time_sync() - t2
